...
## 📊 최종 분석 결과: 하이퍼릴레이션 종합 다이어그램

15개 시나리오를 모두 통합하면, 하나의 금리 결정(2024-10-11 인하)을 중심으로 다음과 같은 **다층적 관계망**이 드러납니다. 이것이 하이퍼릴레이션이 제공하는 핵심 가치 — 단일 쿼리로 "결정의 맥락, 파급, 반론, 후속 영향"을 한눈에 추적할 수 있다는 점입니다.
...
- **decision-chain**: 선행/후행 금리결정을 시간축으로 연결 (간격일수 자동 산출)
- **dissent**: 소수의견 위원·사유·대안금리를 결정에 직접 부착
- **forecast-impact**: 금리결정이 경제전망(GDP·CPI)에 미친 수정 영향과 시차
- **cross-reference**: 금융안정보고서·의사록 간 문서 교차참조와 맥락
- **risk-transmission**: 금리결정 전후 리스크 등급 변화 추적

> 이 다이어그램은 `verify_all_scenarios.py`의 15개 시나리오가 모두 PASS할 때 확인되는 실제 데이터 관계입니다.
